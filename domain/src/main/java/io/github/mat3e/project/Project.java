...
class Project {
    ...
    Set<TaskCreator> convertToTasks(final ZonedDateTime deadline) {
        ...
        var result = steps.stream()
                .map(step -> new TaskCreator(new TaskSourceId(String.valueOf(step.id)), step.description,
    ...
    void updateStep(final int stepId, final boolean taskDone) {
        steps.stream().filter(step -> step.id == stepId).forEach(step -> step.correspondingTaskDone = taskDone);
    }
    ...
    static class Step {
        static Step restore(ProjectStepSnapshot snapshot) {
            return new Step(snapshot.getId(), snapshot.getDescription(), snapshot.getDaysToProjectDeadline(),
                    snapshot.hasCorrespondingTask(), snapshot.isCorrespondingTaskDone());
        }
        ...
        ProjectStepSnapshot getSnapshot() {
            return new ProjectStepSnapshot(id, description, daysToProjectDeadline, hasCorrespondingTask,
                    correspondingTaskDone);
        }
    }
}
